<?php
class Config{
   public static $RT_Mods_dir="C:\games\steam\steamapps\common\BATTLETECH\Mods";

   #These are meant for developers
   public static $debug=FALSE;
   public static $info=FALSE;
   //'mechdef_leviathan_LVT-C';//'mechdef_emperor_EMP-6Z';'mechdef_obsidian_skull_AS-IIC-OS';'mechdef_highlander_HGN-QS3';'mechdef_kugelblitz_SNV-KB';'mechdef_arctic_fox_af1a'
   public static $debug_single_mech=null;
}